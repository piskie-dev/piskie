...
export class ImageFormatError extends Error {
  constructor(readonly code: 'invalidImage' | 'pixels') {
    super(code === 'pixels' ? 'Image dimensions exceed the pixel limit' : 'Invalid image');
    this.name = 'ImageFormatError';
  }
}

/** Reads encoded raster dimensions without decoding or changing the original bytes. */
export function inspectRasterImage(bytes: Uint8Array, maxPixels = Infinity): { mediaType: string; width: number; height: number } {
  const checkDimensions = (width: number, height: number): void => {
    if (width <= 0 || height <= 0) throw new ImageFormatError('invalidImage');
    if (width * height > maxPixels) throw new ImageFormatError('pixels');
  };
  ...
  if (bytes.length >= 33 && bytes[0] === 137 && text(1, 7) === 'PNG\r\n\x1a\n'
    ...
    const skip = (count: number) => {
      if (at + count > bytes.length) throw new ImageFormatError('invalidImage');
      at += count;
    };
    const skipColorTable = (packed: number) => { if (packed & 0x80) skip(3 * (1 << ((packed & 7) + 1))); };
    const skipBlocks = () => {
      for (;;) {
        if (at >= bytes.length) throw new ImageFormatError('invalidImage');
    ...
    while (at < bytes.length) {
      ...
      if (marker === 0x21) {
        ...
        if (at + 9 > bytes.length) throw new ImageFormatError('invalidImage');
        ...
        throw new ImageFormatError('invalidImage');
      }
    }
    ...
    if (view.getUint32(4, true) + 8 !== bytes.length) throw new ImageFormatError('invalidImage');
    const uint24 = (at: number) => bytes[at]! + (bytes[at + 1]! << 8) + (bytes[at + 2]! << 16);
    for (let at = 12; at + 8 <= bytes.length;) {
      const kind = text(at, 4);
      const length = view.getUint32(at + 4, true);
      const data = at + 8;
      if (data + length > bytes.length) throw new ImageFormatError('invalidImage');
    ...
    for (let at = 2; at < bytes.length;) {
      if (bytes[at++] !== 0xff) throw new ImageFormatError('invalidImage');
