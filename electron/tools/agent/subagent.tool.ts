...
const DESCRIPTION = `创建 Worker 承接一个工作包或一个检索问题。subject 是给用户看的短标题。

把新的 Worker 当作一位刚走进房间的聪明同事来交接：它能力完整，可以自主判断；prompt 和 taskIds 指向的看板项是它拿到的全部材料。`;
...
function describeSubagent(types: readonly SubagentTypeDescriptor[]): string {
  ...
  if (!taskBoardTypes.length) return DESCRIPTION;
  return `${DESCRIPTION}
