 ...
 */
async function fetchCdnBytes(url, label, abortSignal) {
    let res;
    try {
 ...
 */
export async function downloadAndDecryptBuffer(encryptedQueryParam, aesKeyBase64, cdnBaseUrl, label, fullUrl, abortSignal) {
 ...
 */
export async function downloadPlainCdnBuffer(encryptedQueryParam, cdnBaseUrl, label, fullUrl, abortSignal) {
    let url;
    if (fullUrl) {
        url = fullUrl;
    }
