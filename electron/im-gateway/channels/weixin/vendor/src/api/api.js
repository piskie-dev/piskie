...
export async function apiGetFetch(params) {
    ...
    try {
        ...
        if (!res.ok) {
            const error = new Error(`${params.label} ${res.status}: ${rawText}`);
...
export async function getUpdates(params) {
    ...
    try {
        ...
        // Internal long-poll timeout is normal; connector abort remains observable.
        if (err instanceof Error && err.name === "AbortError") {
...
export async function sendMessage(params) {
    ...
    if (resp.ret !== undefined && resp.ret !== 0) {
        const error = new Error(`sendMessage ret=${resp.ret} errmsg=${resp.errmsg ?? "(none)"}`);
        error.code = "WEIXIN_SEND_BUSINESS_ERROR";
        error.ret = resp.ret;
        throw error;
    }
}
