...
export async function monitorWeixinProvider(opts) {
    ...
    const configManager = new WeixinConfigManager({ baseUrl, token, abortSignal }, log);
    ...
    while (!abortSignal?.aborted) {
        try {
            ...
            if (abortSignal?.aborted) {
                aLog.info(`Monitor stopped (aborted after getUpdates response)`);
                return;
            }
            ...
            for (const full of list) {
                if (abortSignal?.aborted) {
                    aLog.info(`Monitor stopped (aborted during message processing)`);
                    return;
                }
                ...
                if (abortSignal?.aborted) {
                    aLog.info(`Monitor stopped (aborted after getConfig)`);
                    return;
                }
