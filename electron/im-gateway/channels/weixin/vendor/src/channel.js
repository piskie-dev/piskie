...
import { DEFAULT_ILINK_BOT_TYPE, startWeixinLoginWithQr, waitForWeixinLogin, displayQRCode, submitWeixinLoginVerifyCode, cancelWeixinLogin, } from "./auth/login-qr.js";
...
export const weixinPlugin = {
    ...
    gateway: {
        ...
        loginWithQrStart: async ({ accountId, credentialAccountId, force, verbose }) => {
            // For re-login: use saved baseUrl from account data; fall back to default for new accounts.
            const localAccountId = credentialAccountId
                ? normalizeAccountId(credentialAccountId)
                : undefined;
            const savedBaseUrl = localAccountId ? loadWeixinAccount(localAccountId)?.baseUrl?.trim() : "";
        ...
        loginWithQrWait: async (params) => {
            ...
            const localAccountId = params.credentialAccountId
                ? normalizeAccountId(params.credentialAccountId)
                : undefined;
            const savedBaseUrl = localAccountId
                ? loadWeixinAccount(localAccountId)?.baseUrl?.trim()
                : "";
            ...
            if (result.alreadyConnected) {
                const existing = localAccountId ? loadWeixinAccount(localAccountId) : null;
                if (!params.credentialAccountId || !existing?.token?.trim()) {
                    return {
                        connected: false,
                        state: "error",
                        message: "微信返回已连接状态，但本地账号凭证不存在。",
                    };
                }
                return {
                    connected: true,
                    ...
                    accountId: params.credentialAccountId,
                    message: result.message,
                };
            }
            return {
                connected: result.connected,
                state: result.state ?? (result.connected ? "connected" : "error"),
                message: result.message,
                accountId: result.accountId,
                alreadyConnected: result.alreadyConnected,
            };
        },
        loginWithQrSubmitCode: async (params) => {
            const sessionKey = params.sessionKey || params.accountId || "";
            return submitWeixinLoginVerifyCode(sessionKey, params.code);
        },
        loginWithQrCancel: async (params) => {
            const sessionKey = params.sessionKey || params.accountId || "";
            return cancelWeixinLogin(sessionKey);
        },
    },
};
