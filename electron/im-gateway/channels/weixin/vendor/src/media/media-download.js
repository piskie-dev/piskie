    ...
    if (item.type === MessageItemType.IMAGE) {
        ...
        try {
            const buf = aesKeyBase64
                ...
                : await downloadPlainCdnBuffer(img.media.encrypt_query_param ?? "", cdnBaseUrl, `${label} image-plain`, img.media.full_url, deps.abortSignal);
            ...
            result.decryptedPicPath = `download-failed://${label}-image`;
        }
    }
        ...
        if ((!voice?.media?.encrypt_query_param && !voice?.media?.full_url) || !voice?.media?.aes_key)
            return result;
        try {
            const silkBuf = await downloadAndDecryptBuffer(voice.media.encrypt_query_param ?? "", voice.media.aes_key, cdnBaseUrl, `${label} voice`, voice.media.full_url, deps.abortSignal);
            ...
            result.decryptedVoicePath = `download-failed://${label}-voice`;
        }
    }
        ...
        if ((!fileItem?.media?.encrypt_query_param && !fileItem?.media?.full_url) || !fileItem?.media?.aes_key)
            return result;
        try {
            const buf = await downloadAndDecryptBuffer(fileItem.media.encrypt_query_param ?? "", fileItem.media.aes_key, cdnBaseUrl, `${label} file`, fileItem.media.full_url, deps.abortSignal);
            ...
            result.decryptedFilePath = `download-failed://${label}-file`;
        }
    }
        ...
        if ((!videoItem?.media?.encrypt_query_param && !videoItem?.media?.full_url) || !videoItem?.media?.aes_key)
            return result;
        try {
            const buf = await downloadAndDecryptBuffer(videoItem.media.encrypt_query_param ?? "", videoItem.media.aes_key, cdnBaseUrl, `${label} video`, videoItem.media.full_url, deps.abortSignal);
            ...
            result.decryptedVideoPath = `download-failed://${label}-video`;
        }
    }
    return result;
}
