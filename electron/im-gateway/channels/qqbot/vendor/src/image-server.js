...
/**
 * PISKIE 本地改动（49号 §4.3.1）：下载远程文件到内存 Buffer（不落 vendor 自有目录）。
 *
 * 供入站附件走 ConnectorContext.media.saveBuffer() 直接落受管目录（piskie-media），
 * 复用 downloadFile 的 SSRF 防护、超时与重试语义；maxSizeBytes 为内存安全上限
 * （连接器可比核心媒体上限更严格，超限即失败，不读入完整内容）。
 *
 * @returns { buffer, contentType, error }，buffer 为 null 表示失败
 */
export async function downloadFileToBuffer(url, options) {
    const timeoutMs = options?.timeoutMs ?? 30_000;
    const maxSizeBytes = options?.maxSizeBytes ?? 0; // 0 = 不限制
    const maxRetries = options?.maxRetries ?? 2;
    try {
        ...
        const msg = err instanceof Error ? err.message : String(err);
        console.error(`[image-server] SSRF check failed: ${msg}`);
        return { buffer: null, error: `URL 安全检查未通过: ${msg}` };
    }
    let lastError = null;
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        if (attempt > 0) {
        ...
        lastError = { buffer: null, error: result.error };
        console.error(`[image-server] Attempt ${attempt + 1}/${maxRetries + 1} failed (retryable): ${result.error}`);
    }
    return lastError ?? { buffer: null, error: "下载失败（重试次数耗尽）" };
}
async function downloadToBufferOnce(url, opts) {
    const { timeoutMs, maxSizeBytes } = opts;
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return { buffer: null, error: `下载失败 (${reason})`, retryable: response.status >= 500 };
        }
        if (!response.body) {
            return { buffer: null, error: `下载失败 (响应体为空)`, retryable: false };
        }
        if (maxSizeBytes > 0) {
            ...
            if (contentLength > 0 && contentLength > maxSizeBytes) {
                ...
                return { buffer: null, error: `文件过大（${sizeMB}MB），超过了${limitMB}M的下载限制`, retryable: false };
            }
        }
        ...
        for await (const chunk of Readable.fromWeb(response.body)) {
            ...
            if (maxSizeBytes > 0 && bytesRead > maxSizeBytes) {
                const limitMB = Math.round(maxSizeBytes / (1024 * 1024));
                console.error(`[image-server] Download size exceeded ${limitMB}MB: ${url}`);
                controller.abort();
                return { buffer: null, error: `文件过大，超过了${limitMB}M的下载限制`, retryable: false };
            }
            chunks.push(chunk);
        }
        ...
        if (err instanceof Error && err.name === "AbortError") {
            ...
            return { buffer: null, error: `下载超时（${Math.round(timeoutMs / 1000)}秒）`, retryable: true };
        }
        const retryable = isRetryableNetworkError(err);
        const msg = err instanceof Error ? err.message : String(err);
        console.error(`[image-server] Download error (retryable=${retryable}):`, err);
        return { buffer: null, error: `下载出错: ${msg}`, retryable };
    }
    finally {
        if (timeoutId)
