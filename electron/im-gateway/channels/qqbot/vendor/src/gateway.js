...
export async function startGateway(ctx) {
    ...
    // PISKIE: pre-aborted 检查（49号 §3.2.1）——stop 已请求时不进入启动
    if (abortSignal.aborted) {
        log?.info(`[qqbot:${account.accountId}] start aborted before diagnostics`);
        return;
    }
    // 启动环境诊断（首次连接时执行）
    const diag = await runDiagnostics();
    // PISKIE: 诊断 await 期间可能被 abort——再次检查
    if (abortSignal.aborted) {
        log?.info(`[qqbot:${account.accountId}] start aborted after diagnostics`);
        return;
    }
    ...
    // PISKIE: pre-aborted 处理（49号 §3.2.1）——前面的 await 期间被 abort 时
    // 立即清退已创建的 approvalHandler 等资源并 settle，不进入 connect
    if (abortSignal.aborted) {
        onAbort();
        return;
    }
    ...
    const connect = async () => {
        ...
        try {
            ...
            // PISKIE 本地改动（49号 §3.2.1 abort 契约）：token/gateway URL 两个 await
            // 期间可能被 abort——创建 WebSocket 前复查，停止后不再建立新连接
            if (isAborted || abortSignal.aborted) {
                log?.info(`[qqbot:${account.accountId}] connect aborted after token/gateway fetch, skip WebSocket`);
                isConnecting = false;
                return;
            }
            ...
            const handleMessage = async (event) => {
                ...
                // PISKIE 本地改动（49号 §4.3.1）：移交核心层的附件经 saveMedia 直接落受管目录
                const saveMedia = async (buffer, contentType) => {
                    const saved = await pluginRuntime.channel.media.saveInboundMediaBuffer({
                        accountId: account.accountId,
                        buffer,
                        contentType,
                    });
                ...
                // vendor 生命周期（STT 消费，路径不移交核心），保持上游行为
                const cleanupInboundLocalMedia = () => {
                    const localPaths = [...imageUrls, ...(otherMediaPaths ?? [])].filter((p) => p && !p.startsWith("http://") && !p.startsWith("https://") && !p.startsWith("download-failed://"));
                ...
                if (event.refMsgIdx) {
                    ...
                    if (refEntry) {
                        ...
                        if (refElement) {
                            ...
                            replyToBody = await formatMessageReferenceForAgent(refData, attachmentCtx);
                ...
                // --- 动态上下文 ---
                // PISKIE 本地改动（49号 §4.3.5/§11.3.14/§11.3.26，审3高5）：正文不含
                // 本地路径、远程 URL 或媒体占位符（含计数描述）——图片经
                // ExternalEvent.images 原生视觉注入且纯图片消息保持空正文，语音以
                // 转录文本呈现；构建逻辑见 buildInboundDynamicContext（可单测）
                const dynamicCtx = buildInboundDynamicContext({ asrReferTexts: uniqueVoiceAsrReferTexts });
                ...
                // PISKIE 本地改动（49号 §4.2/§11.3.23）：当前消息不再预加 `[发送者]` 文本前缀，
                // 群消息的发送者身份由核心 InboundPipeline 统一加一次 [IM_GROUP_MEMBER ...] 信封；
                // 合并/引用历史中较早消息的成员标签保留（envelope 历史格式不在此删除范围）。
                ...
                if (isMergedMsg && mergedMessages?.length) {
                    ...
                    // 最后一条消息＝当前消息：不加发送者前缀（49号 §11.3.23，核心信封唯一），仅保留 (@你)
                    const lastContent = formatSubMsgContent(lastMsg);
                    const lastPart = `${lastContent}${isAtYouTag}`;
                    ...
                    userMessage = `${quotePart}${userContent}${isAtYouTag}`;
                }
    ...
    // 开始连接
    await connect();
    // 等待 abort 信号（PISKIE: pre-aborted 时立即 settle，不留永远 pending 的 Promise）
    // PISKIE 本地改动（49号 §3.2.1/§11.1.54）：settle 前有界等待消息队列在途处理
    // 收尾（5s 上限，整体仍在 AccountManager 10s 停止 deadline 内）——fire-and-forget
    // 的 drain 不再绕过 Connector settle barrier
    return new Promise((resolve) => {
        const settle = () => {
            void msgQueue.waitForIdle(5000).then(() => resolve(), () => resolve());
        };
        if (abortSignal.aborted) {
            settle();
            return;
