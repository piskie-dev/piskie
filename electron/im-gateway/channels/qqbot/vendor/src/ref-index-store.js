...
export async function formatMessageReferenceForAgent(ref, ctx) {
    ...
    // PISKIE 本地改动（49号 §4.3.2）：被引用消息的附件只为本次文本格式化（语音转录/
    // 标签）服务，不移交核心 dispatch——格式化完成后立即清理本地落盘文件，防止
    // 受管目录文件因无 ownership handoff 而泄漏（download-failed:// 哨兵非真实路径，跳过）
    const cleanupRefMedia = () => {
        for (const p of attachmentLocalPaths) {
            if (!p || p.startsWith("download-failed://"))
                continue;
            try {
                fs.unlinkSync(p);
            }
    ...
    // 原格式化主体收进闭包，便于 finally 统一清理
    function formatReferenceParts() {
